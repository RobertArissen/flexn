...
export default function RecyclerView({
	...
	const rowRendererWithProps = (
		...
		if (
			vr &&
			(!ClsInstance.getLayouts() ||
				layouts.length !== ClsInstance.getLayouts().length)
		) {
		...
		return rowRenderer(
			...
			data,
			index,
			{ parentContext: ClsInstance, index },
			renderProps
		);
	};
	...
	const flattenContentContainerStyle = StyleSheet.flatten(
		contentContainerStyle
	);
	const flattenStyles = StyleSheet.flatten(style);

	const paddingTop = parseStyleProps(flattenContentContainerStyle?.paddingTop);
	const paddingLeft = parseStyleProps(
		flattenContentContainerStyle?.paddingLeft
	);
	...
	const onLayout = ({ nativeEvent: { layout } }: LayoutChangeEvent) => {
		const unmeasurableDimensions = {
			x:
				...
				marginLeft +
				left +
				(unmeasurableRelativeDimensions.x || 0),
